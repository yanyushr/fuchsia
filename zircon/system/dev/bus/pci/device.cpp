...
zx_status_t Device::Init() {
    ...
    // as plugged in and link ourselves up to the graph. This will in turn
    // keep the device alive as long as the Bus owns it.
...
bool CapabilityCycleExists(Config& cfg,
                           fbl::DoublyLinkedList<fbl::RefPtr<CapabilityBaseType>>* list,
...
zx_status_t AllocateCapability(uint16_t offset,
                               fbl::RefPtr<CapabilityType>* out,
                               fbl::DoublyLinkedList<fbl::RefPtr<typename CapabilityType::BaseClass>>* list) {
    ...
    *out = fbl::AdoptRef(new CapabilityType(static_cast<typename CapabilityType::RegType>(offset)));
    list->push_back(fbl::RefPtr<typename CapabilityType::BaseClass>::Downcast(*out));
    return ZX_OK;
}
...
zx_status_t Device::ParseCapabilities() {
    ...
    while (ReadCapability<uint8_t, PciReg8>(*cfg_, cap_offset, &hdr)) {
        ...
        // things like MSI/PCIE we cache a raw pointer to it for fast access,
        // but otherwise everything is found via the capability list.
        zx_status_t st;
        ...
        case Capability::Id::kFlatteningPortalBridge:
            caps_.list.push_back(fbl::AdoptRef(new Capability(hdr.id, cap_offset)));
            break;
        }
...
zx_status_t Device::ProbeCapabilities() {
    ...
    // TODO(cja): Port extended capabilities
    return ZX_OK;
}
